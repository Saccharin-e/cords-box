...
export function Button({ variant = 'secondary', className = '', style, children, ...props }: ButtonProps) {
  let baseClass = 'btn';
  ...
  const iconStyle: React.CSSProperties = variant === 'icon' ? {
    display: 'flex',
    alignItems: 'center',
  ...
  const ghostStyle: React.CSSProperties = variant === 'ghost' ? {
    background: 'transparent',
    border: 'none',
    cursor: 'pointer',
  } : {};
