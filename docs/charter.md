...
- **Automated Test Coverage:** Vitest tests cover graph solving, WDF adaptors, tone stacks, worklet parity, lint rules, preset libraries, tab parser, MIDI, slot storage, audio pipeline, and UI fretboard components.

Current check results, browser coverage, and limitations are recorded in [VERIFICATION.md](VERIFICATION.md); open findings are tracked in [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md).
